...
bool Player::Create(uint32 guidlow, CharacterCreateInfo* createInfo)
{
    ...
    if (!info)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Player::Create: Possible hacking-attempt: Account %u tried creating a character named '%s' with an invalid race/class pair (%u/%u) - refusing to do so.",
    ...
    if (!cEntry)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Player::Create: Possible hacking-attempt: Account %u tried creating a character named '%s' with an invalid character class (%u) - refusing to do so (wrong DBC-files?)",
    ...
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "Player::Create: Possible hacking-attempt: Account %u tried creating a character named '%s' with an invalid gender (%hu) - refusing to do so",
...
bool Player::BuildEnumData(PreparedQueryResult result, ByteBuffer* dataBuffer, ByteBuffer* bitBuffer)
{
	...
	*dataBuffer << uint8(hairStyle);
	*dataBuffer << uint8(plrRace);                              // Race
	
	dataBuffer->WriteBit(0); //bitBuffer->WriteGuidByte(dataBuffer, 0);
	//bitBuffer->WriteGuildGuidByte(dataBuffer, 4);

	*dataBuffer << uint8(facialHair);                           // Facial hair
	*dataBuffer << uint8(hairColor);                            // Hair color
	*dataBuffer << float(z);                                    // Z

	//bitBuffer->WriteGuildGuidByte(dataBuffer, 6);
	//bitBuffer->WriteGuidByte(dataBuffer, 7);
	//bitBuffer->WriteGuildGuidByte(dataBuffer, 0);

	*dataBuffer << uint32(charFlags);                           // Character flags
	*dataBuffer << uint32(zone);                                // Zone id

	//bitBuffer->WriteGuidByte(dataBuffer, 5);
	//bitBuffer->WriteGuidByte(dataBuffer, 6);

	*dataBuffer << uint32(customizationFlag);                   // Character customization flags
	*dataBuffer << uint32(mapId);                               // Map Id

	//bitBuffer->WriteGuidByte(dataBuffer, 1);

	*dataBuffer << uint32(petDisplayId);                        // Pet DisplayID

	//bitBuffer->WriteGuildGuidByte(dataBuffer, 1);

	*dataBuffer << uint8(face);                                 // Face
	*dataBuffer << uint32(petFamily);                           // Pet family
	*dataBuffer << uint8(skin);                                 // Skin

	//bitBuffer->WriteGuidByte(dataBuffer, 4);
	//bitBuffer->WriteGuildGuidByte(dataBuffer, 5);
	...
	*dataBuffer << float(y);                                    // Y
	...
	//bitBuffer->WriteGuildGuidByte(dataBuffer, 3);
	//bitBuffer->WriteGuildGuidByte(dataBuffer, 7);
	//bitBuffer->WriteGuildGuidByte(dataBuffer, 2);

	*dataBuffer << uint8(level);                                // Level

	//bitBuffer->WriteGuidByte(dataBuffer, 2);
	//bitBuffer->WriteGuidByte(dataBuffer, 3);
...
InstancePlayerBind* Player::GetBoundInstance(uint32 mapid, Difficulty difficulty)
{
	...
	if(difficulty > 3)
	{
		sLog->outInfo(LOG_FILTER_NETWORKIO, "GetBoundInstance trying to GetBoundInstance with difficulty > 3 : map %u diffculty : %u", mapid, difficulty);
		return NULL;
	}
