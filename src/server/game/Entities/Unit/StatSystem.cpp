...
float Player::GetHealthBonusFromStamina()
{
    ...
    float ratio = 10.0f;
	// This is fail. To be done correctly. PandarenCore
	uint32 id = (getClass() - 1) * GT_MAX_LEVEL + getLevel() - 1;
	sLog->outError(LOG_FILTER_NETWORKIO, "GetHealthBonusFromStamina : %u", id);
	gtOCTHpPerStaminaEntry const* hpBase = sGtOCTHpPerStaminaStore.LookupEntry(id);
	if(hpBase)
		ratio = hpBase->ratio;
