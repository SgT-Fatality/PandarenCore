...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (count != 0)
    {
        ...
        do
        {
			...
			bitBuffer.SetGuid(guidLow);
			bitBuffer.SetGuildGuid(fields[13].GetUInt32());

			bitBuffer.WriteGuidMask(4);

            bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(7);
			bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(3);
			bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(0);
			bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(1);
			
			bitBuffer.WriteBit(0); //bitBuffer.WriteBit(0);
            
			bitBuffer.WriteBit(0); //bitBuffer.WriteGuidMask(6);
            bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(6);
            bitBuffer.WriteBit(0); //bitBuffer.WriteGuidMask(1);
			bitBuffer.WriteBits(name.size(), 7);
            bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(2);
            bitBuffer.WriteBit(0); //bitBuffer.WriteGuidMask(2);
			bitBuffer.WriteBit(uint8(guidLow)); // pour le moment car sur 64 BITS normalement !!!! //bitBuffer.WriteGuidMask(0);
			bitBuffer.WriteBit(0); //bitBuffer.WriteGuidMask(3);
			bitBuffer.WriteBit(0); //bitBuffer.WriteGuidMask(5);
            bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(4);
            bitBuffer.WriteBit(0); //bitBuffer.WriteGuidMask(7);
            bitBuffer.WriteBit(0); //bitBuffer.WriteGuildGuidMask(5);
...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recvData)
{
	...
	sLog->outError(LOG_FILTER_NETWORKIO, "gender : %u", gender);
	sLog->outError(LOG_FILTER_NETWORKIO, "skin : %u", skin);
	sLog->outError(LOG_FILTER_NETWORKIO, "face : %u", face);
	sLog->outError(LOG_FILTER_NETWORKIO, "hairStyle : %u", hairStyle);
	sLog->outError(LOG_FILTER_NETWORKIO, "hairColor : %u", hairColor);
	sLog->outError(LOG_FILTER_NETWORKIO, "facialHair : %u", facialHair);

	sLog->outError(LOG_FILTER_NETWORKIO, "====");
	sLog->outError(LOG_FILTER_NETWORKIO, "name : %s", name.c_str());
	sLog->outError(LOG_FILTER_NETWORKIO, "race : %u", race_);
	sLog->outError(LOG_FILTER_NETWORKIO, "class : %u", class_);
	...
	/* PandarenCore - Need to be done via an SQL table because not in DBC anymore
	...
	*/
    ...
    if (AccountMgr::IsPlayerAccount(GetSecurity()))
    {
        ...
        {
			sLog->outError(LOG_FILTER_NETWORKIO, "test 1");
            data << uint8(CHAR_CREATE_DISABLED);
            SendPacket(&data);
            return;
        }
        ...
        {
			sLog->outError(LOG_FILTER_NETWORKIO, "test 2");
            data << uint8(CHAR_CREATE_DISABLED);
            SendPacket(&data);
            return;
        }
    ...
    {
		sLog->outError(LOG_FILTER_NETWORKIO, "test 3");
        data << uint8(res);
        SendPacket(&data);
        return;
    }
    ...
    {
		sLog->outError(LOG_FILTER_NETWORKIO, "test 4");
    ...
    {
		sLog->outError(LOG_FILTER_NETWORKIO, "test 5");
    ...
    {
		sLog->outError(LOG_FILTER_NETWORKIO, "test 6");
    ...
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 3:
        {
            ...
            if (!newChar.Create(sObjectMgr->GenerateLowGuid(HIGHGUID_PLAYER), createInfo))
            {
				...
				sLog->outError(LOG_FILTER_NETWORKIO, "CHAR_CREATE_ERROR 1");
                WorldPacket data(SMSG_CHAR_CREATE, 1);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
	...
	playerGuid = ObjectGuid(recvData.ReadGuid(guidMask, guidBytes));
	playerGuid = 2;
...
{
	sLog->outInfo(LOG_FILTER_GENERAL, "HandlePlayerLogin");
    uint64 playerGuid = holder->GetGuid();
	sLog->outInfo(LOG_FILTER_GENERAL, "playerGUID : %u", playerGuid);
