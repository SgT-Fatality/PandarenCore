...
struct ServerPktHeader
{
    ...
    ServerPktHeader(uint32 size, uint16 cmd) : size(size)
    {
        ...
        if (isLargePacket())
        {
            sLog->outError(LOG_FILTER_NETWORKIO, "initializing large server to client packet. Size: %u, cmd: %u", size, cmd);
...
int WorldSocket::SendPacket(const WorldPacket& pct)
{
	...
	sLog->outInfo(LOG_FILTER_NETWORKIO, "SEND OPCODE [S->C] : %s, LENGTH : %u", GetOpcodeNameForLogging(pct.GetOpcode()).c_str(), pct.size());
    ...
    else
    {
        ...
        {
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::SendPacket enqueue_tail");
            mb->release();
            return -1;
        }
    }
...
int WorldSocket::open(void *a)
{
    ...
    WorldPacket data(MSG_VERIFY_CONNECTIVITY);
...
{
	sLog->outError(LOG_FILTER_NETWORKIO, "SMSG_AUTH_CHALLENGE");
    WorldPacket packet(SMSG_AUTH_CHALLENGE); // 41 ?
    BigNumber seed1;
    seed1.SetRand(16 * 8);
...
int WorldSocket::handle_input(ACE_HANDLE)
{
    ...
    switch (handle_input_missing_data())
    {
        case -1 :
        {
            ...
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::handle_input: Peer error closing connection errno = %s", ACE_OS::strerror(errno));
        ...
        case 0:
        {
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::handle_input: Peer has closed connection");
...
int WorldSocket::handle_input_header(void)
{
    ...
    if (m_Crypt.IsInitialized())
    {
        ...
        {
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::handle_input_header: client sent malformed packet size = %d , cmd = %d",
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    if (opcode >= NUM_MSG_TYPES && opcode != MSG_VERIFY_CONNECTIVITY)
    {
        sLog->outError(LOG_FILTER_NETWORKIO, "SESSION: received nonexistent opcode 0x%.4X", opcode);
        return -1;
    }
	...
	std::string opcodeName = GetOpcodeNameForLogging(new_pct->GetOpcode());
	sLog->outInfo(LOG_FILTER_NETWORKIO, "RECEIVE OPCODE [C->S] : %s, LENGTH : %u", opcodeName.c_str(), new_pct->size());
    ...
    try
    {
        switch(opcode)
        {
            ...
            /*case CMSG_UPDATE_PROGRESS_BAR:
                sLog->outError(LOG_FILTER_NETWORKIO, "CMSG_UPDATE_PROGRESS_BAR, size: " SIZEFMTD " ", new_pct->size());
                return 0;
            */
			default:
            {
                ...
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::ProcessIncoming: Client not authed opcode = %u", opcode);
                    return -1;
                }
            }
        }
    ...
    {
		sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::ProcessIncoming ByteBufferException occured while parsing an instant handled packet (opcode: %u) from client %s, accountid=%i.",
            opcode, GetRemoteAddress().c_str(), m_Session ? m_Session->GetAccountId() : -1);

        sLog->outError(LOG_FILTER_NETWORKIO, "Dumping error-causing packet:");
        new_pct->hexlike();

        sLog->outError(LOG_FILTER_NETWORKIO, "Disconnecting session [account id %i / address %s] for badly formatted packet.",
...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: client build %u, account %s, clientseed %X",
                clientBuild,
                accountName.c_str(),
                clientSeed);
    ...
    /*if (!IsAcceptableClientBuild(clientBuild))
    {
        ...
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Sent Auth Response (version mismatch).");
        return -1;
    }
	*/
    ...
    sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: (s,v) check s: %s v: %s", sStr, vStr);
    ...
    if (fields[3].GetUInt8() == 1) // if ip is locked
    {
        if (strcmp(fields[2].GetCString(), GetRemoteAddress().c_str()))
        {
            ...
            sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Sent Auth Response (Account IP differs).");
            return -1;
        }
    }
...
*/
    ...
    if (memcmp(sha.GetDigest(), digest, 20))
    {
        ...
        sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Sent Auth Response (authentification failed).");
        return -1;
    }
	*/
    std::string address = GetRemoteAddress();

    sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client '%s' authenticated successfully from %s.",
                accountName.c_str(),
                address.c_str());
    ...
    stmt->setString(1, safe_account);

    LoginDatabase.Execute(stmt);

    // NOTE ATM the socket is single-threaded, have this in mind ...
	security = SEC_PLAYER;
    ACE_NEW_RETURN(m_Session, WorldSession(id, this, AccountTypes(security), expansion, mutetime, locale, 0, false), -1);
    ...
    else
    {
        ...
        if (diff_time < ACE_Time_Value(27))
        {
            ...
            if (max_count && m_OverSpeedPings > max_count)
            {
                ...
                {
                    sLog->outError(LOG_FILTER_NETWORKIO, "WorldSocket::HandlePing: Player kicked for "
