...
bool WorldSessionFilter::Process(WorldPacket* packet)
{
	...
	// PandarenCore : Let's ignore unhandled packet
	if(!opHandle)
	{
		sLog->outInfo(LOG_FILTER_NETWORKIO, "Received an UNHANDLED OPCODE : %u", opcode);
		return true;
	}
...
{
    WorldPacket data(SMSG_ACCOUNT_DATA_TIMES, 21);
	...
	for (uint32 i = 0; i <= NUM_ACCOUNT_DATA_TYPES; ++i)
        if ((mask & (1 << i)) != 0)
			if(i == 1 && mask == GLOBAL_CACHE_MASK)
				data << uint32(time(NULL)); //data << uint32(GetAccountData(AccountDataType(i))->Time);// also unix time
			else
				data << uint32(0);
